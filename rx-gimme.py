...
from tkinter import Tk, Grid, Frame, Entry, Button, Label, END, ttk, VERTICAL, Checkbutton, IntVar, messagebox, Text, Scrollbar, filedialog
...
class Window():
    def __init__(self, root):
        ...
        self.label_url.grid(row = self.first_row, column = self.first_column, sticky = "w", padx = 4, pady = 4)
        ...
        self.button_url = Button(frame, text = "Gimme!", width = 10, command=self.gimme)
        self.button_url.grid(row = self.first_row, column = self.first_column + 2)

        self.button_save = Button(frame, text = "Save buffer...", width = 10, command=self.file_save_as)
        self.button_save.grid(row = self.first_row, column = self.first_column + 3)


        self.lines = 40
        self.scrollbar = Scrollbar(frame, orient = "vertical")
        self.text = Text(frame, height = self.lines, yscrollcommand = self.scrollbar.set)
        self.text.grid(row = self.first_row + 1, column = self.first_column, columnspan = 4, sticky = "EW", padx = 4, pady = 4)
        self.scrollbar.grid(row = self.first_row + 1, column = self.first_column + 4, sticky = "NS")
    ...
    def file_save_as(self, event=None, filepath=None):
        if filepath == None:
            filepath = filedialog.asksaveasfilename(filetypes=(('Text files', '*.txt'), ('Python files', '*.py *.pyw'), ('All files', '*.*')))
        try:
